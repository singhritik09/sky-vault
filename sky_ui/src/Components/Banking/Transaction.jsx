...
const Transaction = () => {
    ...
    async function createTransaction(e) {
        ...
        try {
            const response = await axios.post("http://localhost:8000/transaction", {
                amount, senderId, receiverId, password
            })
    ...
    return (
        <div className="flex mt-16 flex-col h-full w-full sm:h-screen md:h-screen mb-0 items-center justify-center px-6 py-8 mx-auto lg:py-2 bg-white" style={{ overflowX: 'hidden' }}>
            <div className="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div className="p-2 py-6 space-y-2 md:space-y-6 sm:p-8">
                    ...
                    <form className="space-y-4 md:space-y-2" method="post" onSubmit={createTransaction} >
                        <div>
                            <label for="email" className="block mb-2 text-sm font-medium text-green-900 dark:text-green-700">Your ID</label>
                            <input
                                value={senderId}
